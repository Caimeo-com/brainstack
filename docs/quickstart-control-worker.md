...
For shared-brain proposal review, create a Telegram topic such as "Proposal curation" and run `/curation`. That binds the current topic to the `proposal-curation` scratch context, points the built-in `brain-curator` routine at this topic, and prints the useful review commands.
