...
When telemux is enabled, `telemux.runtime.env` includes `BRAIN_BASE_URL` and `telemux.secrets.env` includes a blank `BRAIN_IMPORT_TOKEN`. Filling both opts successful runs into shared-brain raw imports of `SUMMARY.md` and `ARTIFACTS.md`; leaving either blank disables the bridge. Built-in curator/update routines are different: they send only a small audit-only execution receipt, not the cumulative scratch summary.

`telemux.runtime.env` also includes `FACTORY_HARNESS`, `FACTORY_HARNESS_BIN`, `FACTORY_TEXT_COALESCE_MS`, `FACTORY_TEXT_COALESCE_RECOVERY_MAX_AGE_MS`, `FACTORY_HARNESS_STREAMING`, and the disabled-by-default `FACTORY_PRE_DISPATCH_CLASSIFIER` knobs. Use `harness.name: claude` and `harness.bin: claude` in `brainstack.yaml` to route jobs through Claude Code instead of Codex by default. Text coalescing merges only short-window plain-text Telegram messages from the same user/chat/topic; commands and attachments flush pending text first. After a restart, stale pending coalesced text is dropped instead of being auto-run long after the user sent it.

Codex-backed Telemux jobs stream a low-noise status card by default after a short delay. Telemux edits that card with safe progress such as session start, command starts/completions, file changes, and completion, then sends the normal final answer separately. It does not forward raw private reasoning. Set `FACTORY_HARNESS_STREAMING=off` to return to typing-heartbeat-only behavior, or tune `FACTORY_HARNESS_STREAMING_INITIAL_DELAY_MS`, `FACTORY_HARNESS_STREAMING_UPDATE_INTERVAL_MS`, and `FACTORY_HARNESS_STREAMING_MAX_CHARS`.
...
Optional Telegram voice/audio transcription is installed through `brainctl capabilities install voice --target <machine>` or Telegram phrases such as `install voice on erbine`. Telemux can receive the voice message on the control host and run the transcriber locally or on a named worker; the resulting text is dispatched as the normal Telegram instruction in the same topic. See [`voice-transcription.md`](./voice-transcription.md).

Run `/explainctx` in an unbound topic to see what binding means before creating a context. Run bare `/newctx` for a guided deterministic binding flow: telemux suggests a slug from the Telegram topic title when Telegram includes it, falls back to `topic-<thread-id>` when it does not, then asks for machine and use-case choices. The target picker is intentionally phrased by intent: topic workspace (`scratch`), machine administration (`host`), or code repository/path. For code repos, reply `3` to be asked for the repo, or reply directly with `repo <url|org/repo|path> [base-branch]`. GitHub shorthand such as `lindy-ai/lindy main` is expanded to the clone URL automatically. The full positional form is `/newctx <slug> <machine> <scratch|host|repo-url|org/repo|path> [base-branch]`.

For shared-brain proposal review, create a Telegram topic such as "Proposal curation" and run `/curation`. That binds the current topic to the `proposal-curation` scratch context, points the built-in `brain-curator` routine at this topic, and prints the useful review commands. You can also say "show proposal merge candidates" or "look for proposal merges" in that topic; telemux runs the harness batch scan on the control host over the top 100 open proposals, submits consolidated merge proposals, and does not apply wiki edits.
