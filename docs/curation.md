...
Telegram Accept applies the proposed wiki change when the proposal carries one (drift still parks it as `needs-human`). Context-only candidates without a target page must be enriched or merged before they can be accepted. `/proposal_groups` shows review groups with multiple open proposals. `/proposal_merges` runs the bounded safe consolidation pass (`brainctl proposals auto-merge --submit --max-group-size 6 --limit-groups 5`): it creates consolidated proposals and marks absorbed sources superseded, but does not apply wiki edits. Say "look for proposal merges", "show proposal merge candidates", or "merge related proposals" instead of using the commands directly. Accept/reject and merge submission from Telegram require the optional `FACTORY_BRAIN_ADMIN_TOKEN` in the telemux env; without it, Telegram stays read-only for proposals.
