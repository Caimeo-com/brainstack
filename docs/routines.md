...
Inspect and drive it with `/curation`, `/curator_status`, `/curator_run`, filtered `/proposals`, `/proposal_groups`, and `/proposal_merges` commands in Telegram, or `brainctl curator status|run|install` from the control host. Plain phrases such as "show proposal merge candidates" and "look for proposal merges" work too. Run `/curation` in a dedicated proposal-review topic to bind that topic to the `proposal-curation` scratch context and move the built-in `brain-curator` routine there without creating duplicates. A scheduled curator run is already inside the curator context, so it should perform the pass directly rather than calling `brainctl curator run` again.
