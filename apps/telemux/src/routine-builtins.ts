...
function brainCuratorInstruction(): string {
  return [
    ...
    "",
    "Checklist:",
    "1. Read `brainctl curator inbox --json` first. It applies Brainstack's deterministic evidence policy from source manifests and returns the only raw evidence candidates this pass should inspect. Keep audit-only receipts as provenance; do not read their bodies or propose source pages for them.",
    ...
    "6. Run `brainctl proposals auto-merge --submit --json --max-group-size 6 --limit-groups 5` to consolidate safe related date/topic batches inside review groups. This creates consolidated proposals and marks absorbed source candidates superseded; it must not apply wiki edits. Leave skipped, broad, legacy, context-poor, or one-off batches for operator review.",
    "7. Risk guidance: low = additive, sourced, small (status/source pages); medium = restructures or edits prose; high = deletes content, changes decisions, or touches runbooks.",
    "8. Produce a sourced curation report with: eligible evidence processed, audit-only receipt count, submitted proposals, auto-merged groups, contradictions, stale facts, skipped private-sensitive items, and exact candidate source paths/ids.",
    "9. Update `.factory/SUMMARY.md`, `.factory/TODO.md`, and `.factory/ARTIFACTS.md` so the next curator pass can resume.",
    "",
    "Telegram summary:",
