...
- Use `brainctl context-packs put --machine <machine> --name <name> --dir <folder> --dry-run` to preflight an evolving local folder before syncing it as reusable private context. Then run without `--dry-run`, attach it with `brainctl context-packs attach --context <slug> --machine <machine> --name <name>`, and refresh it from the source machine with `brainctl context-packs sync --machine <machine> --name <name>`. Folder Packs are private machine state; prompts receive only folder paths and metadata, not contents.
